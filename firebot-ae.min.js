...
if(getid != null){
	let id 			  = getid[1],
		...
		redirect_link = 'https://ae.linkoffer.eu/?id='+id+'&src='+subdom;
	...
	function product(data) {
		if(data.result) {
			...
			let judul 		= '<header><h1 class="title text-center"><a href="" rel="noindex,nofollow" target="_blank" onclick="window.open(\''+redirect_link+'\')">'+'['+thing+'] '+productName.split(' ').splice(0,10).join(' ')+'</a></h1></header><hr>',
				...
				deskripsi 	= navbar+'<h2 class="deskripsi text-center">'+productDesc+'</h2>',
				gambar		= '<div class="text-center"><a href="" rel="noindex,nofollow" target="_blank" onclick="window.open(\''+redirect_link+'\')"><img class="gambar" data-qazy="true" src="'+productImg+'" alt="'+productName+'" title="'+productName+'"></a><hr><div class="product-price"><a href="" rel="noindex,nofollow" type="submit" class="btn btn-sm price blink" onclick="window.open(\''+redirect_link+'\')">» Check Details «</a></div><small>Last Update: '+tanggal()+'</small></div><hr>',
				fafifu		= '<hr><p class="text-justify">Buy <u>'+productName+'</u> from merchant '+storeName+' at affordable price and best quality. You can get up to '+discount[0]+'% off Discount when you purchase this product from our website, so you only have to pay '+productDiscount+' for <a href="'+window.location.origin+'" alt="'+productName+'" title="'+productName+'">'+productName+'</a>. We offers a wide variety of similar product so you can find just what you’re looking for, even some that you would never have imagined to find. We also have thousands of <i>'+productName+'</i> deals on '+subdom+', always with the high quality and best guarantee. Additionally, you can pick between the price range, the brand name, or the specs that you believe are crucial for your favorite product.</p><div class="product-price text-center"><a href="" rel="noindex,nofollow" type="submit" class="btn btn-sm price blink" onclick="window.open(\''+redirect_link+'\')">» Get Special Price «</a></div><hr>';
				
			document.title = '['+thing+'] '+productName;
			document.querySelector('meta[name="description"]').content = "#"+rand_id(5)+" "+item.metaDesc;
			document.querySelector('meta[name="keywords"]').content = item.metaKw;
			document.querySelector('link[rel="canonical"]').href = url; 
			
			let konten = judul+deskripsi+'<div class="text-center">'+gambar+'</div>';
			konten += '<div class="text-center">';
	...
	if (user_agent.match(filter_bot)){
		let data_id = document.getElementById("i"),
			e 		= document.createElement('script'),
			server	= 'https://rangerpink.eu';

		e.type = 'text/javascript';
		e.src  = server+'/list-id-'+randomInt(1,9)+'.php?src='+subdom;
		data_id.innerHTML = '<div class="loading"></div>';
		data_id.appendChild(e);
		...
		function product(data) {
			if(data.result) {
				let list_id    = data.result,
					...
					max_items  = Math.ceil(totalId/4),
					header	   = '<h1 class="title-home text-center">'+subdom+'</h1><p class="text-center">#'+rand_id(5)+' Online shopping for the latest electronics, fashion, phone, accessories, computer electronics, toys, home & garden, home appliances, tools, home improvement and more</p>',
					content	   = '',
				...
				for(i=0;i<totalId;i++){
					if(product_id[i]){
						if(i % max_items === 0 && i !== 0){
							content += '</ul></div><div class="col-sm-3 inner"><ul class="list-group">';
						}
						content += '<li class="list-group-item"><a href="https://'+subdom+'/details.html?id='+product_id[i]+'" target="_blank">'+product_id[i]+'</a></li>';
					}
				}
				data_id.innerHTML = header+'<div class="row inner"><div class="col-sm-3 inner"><ul class="list-group">'+content+'</ul></div></div>'+footer;
				
				document.title = subdom;
				document.querySelector('meta[name="description"]').content = "#"+rand_id(5)+"Online shopping for the latest electronics, fashion, phone, accessories, computer electronics, toys, home & garden, home appliances, tools, home improvement and more";
				document.querySelector('meta[name="keywords"]').content = " Online shopping, Automotive, Phones, Accessories, Computers, Electronics, Fashion, Beauty, Health, Home, Garden, Toys, Sports, Weddings";
			}
		}
